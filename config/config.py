...
class AppConfig:
    ...
    def validate(self) -> bool:
        ...
        Note: OpenAI API key is optional - system can work without it,
        but AI-powered features (candidate evaluation, message generation) won't work.
        """
        issues = []
        warnings = []
        
        # Check critical API keys (required)
        if not self.linkedin.api_key:
            issues.append("LINKEDIN_API_KEY is not configured")
        
        if not self.linkedin.account_id or self.linkedin.account_id == "your_linkedin_account_id_here":
            issues.append("LINKEDIN_ACCOUNT_ID is not configured")
        
        # Check MongoDB connection (required for database operations)
        ...
        # OpenAI API key is optional (warning, not error)
        if not self.openai.api_key or self.openai.api_key == "your_openai_api_key_here":
            warnings.append("OPENAI_API_KEY is not configured (AI features will be disabled)")
